"""CPM CDC extract — migration of workflow wf_CPM_CDC.

Reads CPM_NEWPAY_TBL for the current pay period, renders the NIH payroll
master fixed-width file (layout: cdcskel_WS_PAY_OUT_REC) with header from
cdchdr_WS_CDC_HDR, writes the file to the staging directory and mirrors the
records into CPM_CDC_STG_TBL.
"""
import argparse
import logging
import os
...
logger = logging.getLogger("cpm.cdc")

AGENCY = "CDC"
MODULE = "CPM_CDC"
MASTER_DEF = "cdcskel_WS_PAY_OUT_REC"
STAGING_TABLE = "CPM_CDC_STG_TBL"
OUTPUT_FILE = "cpm_cdc_payroll.txt"


def run(env: str = None, run_date: str = None) -> int:
    config = load_config(env)
    secret = get_secret("biis", config)
    spark = get_spark("biis-cpm-cdc")
    try:
