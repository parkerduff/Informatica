...
Replaces Informatica PowerCenter workflow for CPM OIG processing
...
with DAG(
    ...
    description="CPM OIG Build Message — filter, aggregate, and email notification.",
    schedule_interval="@daily",
    start_date=datetime(2018, 9, 28),
    catchup=False,
    tags=["cpm", "oig", "etl", "pyspark"],
) as dag:

    pp_num = Variable.get("MAP_PP_NUM")
    pp_end_year = Variable.get("MAP_PP_END_YEAR")

    cpm_oig_job = SparkSubmitOperator(
        task_id="cpm_oig_build_message",
        application="pyspark_migration/jobs/cpm_oig.py",
        application_args=[
            "--pp-end-year", str(pp_end_year),
            "--pp-num", str(pp_num),
        ],
        name="CPM_OIG_Build_Message",
