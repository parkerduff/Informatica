"""
PySpark migration of Informatica workflow: CPM_NIH
Source XML: XML/CPM_NIH

CPM Agency Extract for NIH. Reads from CPM_NEWPAY_TBL (keyed on PP_END_YEAR,
PP_NUM, DFAS_PSEUDO_SSN, LINE_TYPE) with NIH-specific agency filters,
processes VSAM source files (PC_DOEYTD_RDF.TXT, PC_DOEMER_RDF.TXT),
...
# Output file paths
OUTPUT_FILE_NEWPAY = os.path.join(TARGET_FILE_DIR, "CPM", "CPM.NIH.TEST.DAT.TXT")
OUTPUT_FILE_YTD = os.path.join(TARGET_FILE_DIR, "CPM", "CPM.NIH.YTD.DAT.TXT")
OUTPUT_FILE_MER = os.path.join(TARGET_FILE_DIR, "CPM", "CPM.NIH.MER.DAT.TXT")
OUTPUT_FILE_PAD = os.path.join(TARGET_FILE_DIR, "CPM", "CPM.NIH.PAD.DAT.TXT")


# ---------------------------------------------------------------------------
# COMP-3 Packed Decimal Helpers
# ---------------------------------------------------------------------------
def decode_comp3(packed_bytes: bytes) -> float:
    """Decode COBOL COMP-3 packed decimal bytes to a Python float.

    COMP-3 stores each digit in a nibble (4 bits) with the last nibble
    being the sign (C = positive, D = negative, F = unsigned positive).
    """
    if not packed_bytes:
        return 0.0
    ...
    # Build the number from remaining digits
...
def read_vsam_file(
    ...
    """Read a VSAM-format file (COMP-3 packed decimal) as text.

    Since PySpark has no native VSAM reader, we read the file as text
    and parse it. In production, a COBOL copybook parser (e.g., cobrix)
    should be used for accurate field extraction.

    Falls back to reading as text with pipe delimiter if pre-converted.
    """
    logger.info("Reading VSAM file: %s", file_path)

    if not os.path.isfile(file_path):
        logger.warning("VSAM file not found: %s", file_path)
        return spark.createDataFrame([], StructType([
            StructField("RECORD", StringType(), True),
        ]))

    # Try to read as pre-converted text first (recommended approach)
    try:
        df = spark.read.text(file_path)
        logger.info("Read VSAM file as text: %d row(s)", df.count())
        return df
    except Exception as exc:
        logger.warning("Failed to read VSAM file %s: %s", file_path, exc)
        return spark.createDataFrame([], StructType([
            StructField("RECORD", StringType(), True),
        ]))
...
def step_set_pay_calendar(
    ...
    """Read CPM_CYCLE_TBL for this agency to set pay period context.
...
# ===================================================================
# Step 2: Read CPM_NEWPAY_TBL with agency filter
# ===================================================================
def step_read_newpay(
    ...
    """Read CPM_NEWPAY_TBL filtered for NIH agency records.

    Key columns: PP_END_YEAR, PP_NUM, DFAS_PSEUDO_SSN, LINE_TYPE.
    """
...
def step_process_vsam_files(
    ...
    """Process VSAM source files (YTD and MER).

    PC_DOEYTD_RDF.TXT - Year-to-date detail records
    PC_DOEMER_RDF.TXT - MER records

    These files use mainframe COMP-3 packed decimal (IBMCOMP=YES).
    In production, pre-convert using a COBOL copybook parser.
    """
    logger.info("Step 3: Process VSAM source files")
...
def step_validate_and_output(
    ...
    """Validate records against staging tables and write output files.
    ...
    # --- Validation against staging tables ---
    error_count = 0
...
# Main workflow
# ===================================================================
def run_cpm_nih() -> None:
    """Execute the full CPM_NIH workflow."""
